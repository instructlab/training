...
def main(args):
    ...
    print("model type", model_conf.model_type)
    ...
    # would like to replace this with either
    # a) a dataset class
    ...
    m = Model(
        ...
        lora_r=args.lora_r,
        distributed_framework=args.distributed_training_framework,
        tokenizer=tokenizer,
    ...
    # accelerator does not need optimizer to init
    ...
    # 
  #  m.update_model(accelerator.model)
    optimizer = Optimizer(
        model=m,
    ...
    accelerator.prepare_with_optimizer(optimizer=optimizer, lr_scheduler=args.lr_scheduler, num_epochs=args.num_epochs, num_warmup_steps=args.num_warmup_steps)
    # this is probably uncessary
   # m.update_model(accelerator.model)
  #  optimizer.update_optimizer(accelerator.optimizer)
