...
class Model:
    ...
    #     # If model is another Model instance, get its underlying model
    ...
    def train(self, mode=True):
        ...
        """
       # print("BAR",isinstance(self.model, Model))
    ...
    def __getattr__(self, name):
        # Don't try to delegate `model` — we already have it
        if name == 'model':
    ...
    #     This method is called when an attribute is not found in the Model class.
    #     It forwards the request to the underlying model.
    #     """
    #     try:
    #         return getattr(self.model, name)
    ...
    def create_lora_config(self):
        ...
        if not self.lora_target_modules:
            ...
            requested = set(self.lora_target_modules)
            ...
            if not valid:
                raise ValueError(
                    ...
                    f"Requested: {self.lora_target_modules}\nAvailable: {proj_layers}"
                )
            if missing:
                print(
            ...
            self.lora_target_modules = list(valid)
...
class Optimizer(TorchOptim):
    def __init__(
        ...
        self.distributed_framework = model.distributed_framework
        if name is not None:
            ...
            if model.distributed_framework == DistributedBackend.FSDP.value:
                self.optimizer = AdamW(
                    model.parameters(), lr=learning_rate, betas=betas
                )
            elif model.distributed_framework == DistributedBackend.DEEPSPEED.value:
                if cpu_offload:
                    self.optimizer = DeepSpeedCPUAdam(
                        model.parameters(), lr=learning_rate, betas=betas
                    )
...
class Accelerator:
    ...
    def prepare_with_optimizer(self, optimizer: Optimizer, lr_scheduler: str, num_epochs: int, num_warmup_steps: int):
        self.setup_lr_scheduler(optimizer=optimizer, lr_scheduler=lr_scheduler, num_epochs=num_epochs, num_warmup_steps=num_warmup_steps)
        ...
        self.model.update_model(new_m)
        optimizer.update_optimizer(new_optimizer=new_opt)
        self.optimizer = optimizer

    def setup_lr_scheduler(self, optimizer: Optimizer, lr_scheduler: str, num_epochs: int, num_warmup_steps: int):
    ...
    def __call__(self, *args, **kwargs):
        return self.accelerator(*args, **kwargs)
...
class Checkpointer:
    def __init__(self, model=Model, optimizer=Optimizer, accelerator=Accelerator, strategy="full_state"):
    ...
    def save_fsdp_lora_model(
        ...
        if accelerator.is_main_process:
            ...
            model_copy = AutoModelForCausalLM.from_pretrained(
                **model.basemodel_args, device_map="cpu"
            )
            ...
            if old_device_map:
                ...
                model.basemodel_args["device_map"] = old_device_map
    ...
    def save_hf_format_accelerate(
        ...
        subdir = (
            "last_epoch" if last_epoch else f"samples_{samples_seen}"
        )
        ...
        is_fsdp_lora = (
            self.model.lora_r is not None and self.accelerator.distributed_type == DistributedBackend.FSDP
        )
        if is_fsdp_lora:
        ...
        if self.model.model_type == "dolomite" and convert_dolomite and self.accelerator.is_main_process:
            # export doesnt like the directory to exist
            if final_output_dir.exists():
                shutil.rmtree(final_output_dir)
            export_to_huggingface(
                pretrainedmodel_name_or_path=tmpdir.name,
